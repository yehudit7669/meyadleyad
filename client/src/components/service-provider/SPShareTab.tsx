...
const SPShareTab: React.FC<Props> = ({ profile }) => {
  ...
  const providerPageUrl = `${baseUrl}/providers/${profile.id}`;

  // Fetch service provider ads
  ...
  const handleCopyProviderLink = async () => {
    try {
      await navigator.clipboard.writeText(providerPageUrl);
      setCopiedId('provider-page');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {copiedId === 'provider-page' ? '✓ הועתק!' : 'העתק קישור'}
          </button>
        </div>
