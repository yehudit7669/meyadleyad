...
const SPAuditLogTab: React.FC = () => {
  const { data: logs, isLoading } = useQuery({
    queryKey: ['service-provider-audit-log'],
    queryFn: () => serviceProviderService.getAuditLog(100),
  });

  const getActionLabel = (action: string) => {
    const labels: Record<string, string> = {
      UPDATE_PROFILE: 'עדכון פרטים אישיים',
      UPDATE_SP_PROFILE: 'עדכון פרטי נותן שירות',
      ...
      REQUEST_OFFICE_CHANGE: 'בקשת שינוי כתובת משרד',
      REQUEST_DATA_EXPORT: 'בקשת ייצוא נתונים',
      REQUEST_ACCOUNT_DELETE: 'בקשת מחיקת חשבון',
      REQUEST_HIGHLIGHT: 'בקשה להבלטה',
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">לוג מערכת (Audit Log)</h2>
