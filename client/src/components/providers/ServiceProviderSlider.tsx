...
interface ServiceProviderSliderProps {
  cityId?: string;
}

const ServiceProviderSlider: React.FC<ServiceProviderSliderProps> = ({ cityId }) => {
  ...
  if (!isLoading && providersList.length === 0) {
    return null;
  }
          ...
          >
            {isLoading ? (
              ...
              providersList.map((provider: any) => (
                <div key={provider.id} className="min-w-[200px] max-w-[200px]">
