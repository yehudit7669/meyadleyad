...
import { AdType, WantedSharedOwnershipWizardData } from '../../../types/wizard';
import WizardProgress from '../WizardProgress';
import WantedSharedOwnershipStep1 from './WantedSharedOwnershipStep1';
import WantedSharedOwnershipStep2 from './WantedSharedOwnershipStep2';
import WantedSharedOwnershipStep3 from './WantedSharedOwnershipStep3';
import WantedSharedOwnershipStep4 from './WantedSharedOwnershipStep4';
import WantedSharedOwnershipPreview from './WantedSharedOwnershipPreview';

const WantedSharedOwnershipWizard: React.FC = () => {
  const navigate = useNavigate();
  const [currentStep, setCurrentStep] = useState(1);
  const [wizardData, setWizardData] = useState<Partial<WantedSharedOwnershipWizardData>>({});
  ...
  const handleFinalSubmit = async (sendCopyToEmail: boolean) => {
    console.log('🚀 handleFinalSubmit called - WantedSharedOwnership');
    ...
    try {
      const payload = prepareApiPayload(wizardData as WantedSharedOwnershipWizardData, sendCopyToEmail);
  ...
  const prepareApiPayload = (data: WantedSharedOwnershipWizardData, sendCopyToEmail: boolean) => {
    ...
    let category = categories?.find((cat: any) => cat.slug === 'wanted-shared-ownership');
    if (!category) {
      category = categories?.find((cat: any) => cat.nameHe === 'דרושים - טאבו משותף');
    }
    ...
    if (!categoryId) {
      ...
      throw new Error('לא נמצאה קטגוריה דרושים - טאבו משותף. אנא וודא שהקטגוריה קיימת במערכת.');
    }
    ...
    const title = `מחפש ${getRoomsLabel(step3.rooms)} חדרים ב${locationText}`;
    ...
    <div className="min-h-screen bg-gray-50 py-8" dir="rtl">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="bg-white rounded-lg shadow-md p-8">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-[#1F3F3A] mb-2">דרושים - טאבו משותף</h1>
...
export default WantedSharedOwnershipWizard;
