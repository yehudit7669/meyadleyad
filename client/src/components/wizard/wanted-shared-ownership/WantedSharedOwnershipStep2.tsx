...
import { SharedOwnershipStep2Data, sharedOwnershipStep2Schema, WizardStepProps } from '../../../types/wizard';

const WantedSharedOwnershipStep2: React.FC<WizardStepProps> = ({ data, onNext, onPrev }) => {
  const [formData, setFormData] = useState<SharedOwnershipStep2Data>(
    data || {
      cityId: '',
      cityName: '',
      streetId: undefined,
  ...
  const handleChange = (field: keyof SharedOwnershipStep2Data, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleNext = () => {
    try {
      sharedOwnershipStep2Schema.parse(formData);
      onNext(formData);
    } catch (error: any) {
...
export default WantedSharedOwnershipStep2;
