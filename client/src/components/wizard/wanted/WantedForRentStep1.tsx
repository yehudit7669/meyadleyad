...
import { WantedForRentStep1Data, wantedForRentStep1Schema } from '../../../types/wizard';

interface Props {
  data?: WantedForRentStep1Data;
  onNext: (data: WantedForRentStep1Data) => void;
  onPrev: () => void;
  isFirst?: boolean;
}

const WantedForRentStep1: React.FC<Props> = ({ data, onNext }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formData: WantedForRentStep1Data = { hasBroker };

    try {
      wantedForRentStep1Schema.parse(formData);
      onNext(formData);
    } catch (error: any) {
...
export default WantedForRentStep1;
