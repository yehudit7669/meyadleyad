...
const WantedHolidayStep1: React.FC<Props> = ({ data, onNext }) => {
  const [formData, setFormData] = useState<WantedHolidayStep1Data>(
    data || {
      cityId: '',
      cityName: '',
      streetId: undefined,
  ...
  const handleChange = (field: keyof WantedHolidayStep1Data, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 animate-fadeIn" dir="rtl">
      <div>
        <h2 className="text-2xl font-bold text-[#1F3F3A] mb-2">כתובת/אזור מבוקש</h2>
        <p className="text-gray-600">הזן את כתובת הדירה המבוקשת</p>
      </div>
