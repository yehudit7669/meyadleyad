...
interface Props {
  ...
  onNext: (data: WantedForSaleStep4Data) => void;
  onPrev: () => void;
  isLast?: boolean;
  isLoading?: boolean;
}
...
const WantedForSaleStep4: React.FC<Props> = ({ data, onNext, onPrev, isLoading }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formData: WantedForSaleStep4Data = {
    ...
    try {
      wantedForSaleStep4Schema.parse(formData);
      onNext(formData);
    } catch (error: any) {
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <label className="flex items-start cursor-pointer">
          <input
            ...
            className="mt-1 ml-3 w-5 h-5 text-green-600 border-gray-300 rounded focus:ring-green-500"
          />
...
export default WantedForSaleStep4;
