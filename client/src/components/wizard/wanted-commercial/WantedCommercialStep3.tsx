...
export interface WantedCommercialStep3Data {
  cityId: string;
  cityName: string;
  streetId?: string;
  streetName?: string;
...
const WantedCommercialStep3: React.FC<WizardStepProps> = ({ data, onNext, onPrev }) => {
  const [formData, setFormData] = useState<WantedCommercialStep3Data>(
    data || {
      cityId: '',
      cityName: '',
      streetId: undefined,
  ...
  const handleChange = (field: keyof WantedCommercialStep3Data, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
...
export default WantedCommercialStep3;
