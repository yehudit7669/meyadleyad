...
import WantedCommercialStep1 from './WantedCommercialStep1';
import WantedCommercialStep2 from './WantedCommercialStep2';
import WantedCommercialStep3 from './WantedCommercialStep3';
import WantedCommercialStep4 from './WantedCommercialStep4';
import WantedCommercialStep5 from './WantedCommercialStep5';
import WantedCommercialPreview from './WantedCommercialPreview';
...
const WantedCommercialWizard: React.FC = () => {
  const navigate = useNavigate();
  const [currentStep, setCurrentStep] = useState(1);
  const [wizardData, setWizardData] = useState<WantedCommercialWizardData>({});
  ...
  const handleFinalSubmit = async (sendCopyToEmail: boolean) => {
    console.log('🚀 handleFinalSubmit called - WantedCommercial');
  ...
  const prepareApiPayload = (data: WantedCommercialWizardData, sendCopyToEmail: boolean) => {
    ...
    // Find categoryId for wanted-commercial
    let category = categories?.find((cat: any) => cat.slug === 'wanted-commercial');
    if (!category) {
      category = categories?.find((cat: any) => cat.nameHe === 'דרושים - נדלן מסחרי');
    }
    ...
    if (!categoryId) {
      ...
      throw new Error('לא נמצאה קטגוריה דרושים - נדלן מסחרי. אנא וודא שהקטגוריה קיימת במערכת.');
    }
    ...
    const transactionLabel = step1.transactionType === 'FOR_RENT' ? 'מחפש להשכיר' : 'מחפש לקנות';
    const title = `${transactionLabel} ${typeLabel} ב${locationText}`;
    ...
    <div className="min-h-screen bg-gray-50 py-8" dir="rtl">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="bg-white rounded-lg shadow-md p-8">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-[#1F3F3A] mb-2">דרושים - נדלן מסחרי</h1>
...
export default WantedCommercialWizard;
