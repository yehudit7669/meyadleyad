/**
 * Google Forms Apps Script - אינטגרציה עם מערכת המקום (טאבו משותף)
 * 
 * הוראות התקנה:
 * 1. פתח את טופס טאבו משותף ב-Google Forms
...
const CATEGORY = 'טאבו משותף';  // שם הקטגוריה (חייב להיות בדיוק כמו בבסיס הנתונים!)
...
const FIELD_MAPPING = {
  ...
  // פרטי הנכס
  propertyType: 'סוג הנכס',  // בית פרטי, דירת גן, דופלקס וכו'
  rooms: 'מספר חדרים',
  ...
  // שדות ייחודיים לטאבו משותף
  broker: 'תיווך',  // עם תיווך / ללא תיווך
  requiredEquity: 'הון עצמי דרוש',
  numberOfPartners: 'מספר שותפים',
...
function buildPayload(responses) {
  ...
  const numberOfPartners = getFieldValue(responses, FIELD_MAPPING.numberOfPartners);
  
  let autoTitle = '';
  if (propertyType) autoTitle += propertyType + ' ';
  if (rooms) autoTitle += rooms + ' חדרים ';
  if (numberOfPartners) autoTitle += '(טאבו משותף - ' + numberOfPartners + ' שותפים) ';
  if (city) autoTitle += 'ב' + city;
  if (!autoTitle) autoTitle = 'נכס בטאבו משותף';
  ...
  const payload = {
    ...
    category: CATEGORY,
    title: autoTitle,
    description: '', // נוסיף תיאור אוטומטי
  };
  ...
  if (propertyType) {
    ...
    else if (propertyType.includes('יחידת דיור') || propertyType.includes('יחידה')) mappedType = 'UNIT';
  ...
  // הון עצמי דרוש (שדה ייחודי לטאבו משותף)
  const requiredEquity = getFieldValue(responses, FIELD_MAPPING.requiredEquity);
  if (requiredEquity) {
    const cleanEquity = requiredEquity.toString().replace(/[^\d.]/g, '');
    customFields.requiredEquity = parseFloat(cleanEquity);
  }
  
  // מספר שותפים (שדה ייחודי לטאבו משותף)
  if (numberOfPartners) {
    const cleanPartners = numberOfPartners.toString().replace(/[^\d]/g, '');
    customFields.numberOfPartners = parseInt(cleanPartners);
  }
  ...
  if (features) {
    ...
    if (Array.isArray(features)) {
      ...
      featuresObj.accessibleForDisabled = features.includes('נגישות') || features.includes('גישה לנכים');
      ...
      featuresObj.accessibleForDisabled = featuresArray.some(f => f.includes('נגישות') || f.includes('גישה לנכים'));
    }
  ...
  // בניית תיאור אוטומטי מהנתונים
  let autoDescription = 'נכס בטאבו משותף';
  if (numberOfPartners) autoDescription += ' ל-' + numberOfPartners + ' שותפים';
  if (requiredEquity) {
    const equityFormatted = parseFloat(requiredEquity.toString().replace(/[^\d.]/g, '')).toLocaleString('he-IL');
    autoDescription += ', הון עצמי דרוש: ₪' + equityFormatted;
  }
  if (rooms) autoDescription += ', ' + rooms + ' חדרים';
  if (squareMeters) autoDescription += ', שטח: ' + squareMeters + ' מ"ר';
  if (floor) autoDescription += ', קומה ' + floor;
  
  payload.description = autoDescription;
...
function testSubmission() {
  ...
  const testResponses = {
    ...
    'וועד בית (לחודש)': '350',
    'תיווך': 'ללא תיווך',
    'הון עצמי דרוש': '300000',
    'מספר שותפים': '3',
    'מאפיינים': ['חניה', 'מחסן', 'מיזוג', 'ממ"ד', 'מעלית', 'מטבח משודרג'],
